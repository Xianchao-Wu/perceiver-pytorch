...
from perceiver_pytorch.perceiver_pytorch import exists, default, cache_fn, \
    fourier_encode, PreNorm, FeedForward, Attention
...
class Perceiver(nn.Module):
    def __init__(
        ...
        *,
        ...
        get_input_attn = lambda: PreNorm(input_dim, LinearAttention(input_dim, 
                                                                    dropout = attn_dropout))
        get_rev_cross_attn = lambda: PreNorm(input_dim, Attention(input_dim, 
                                                                  latent_dim, heads = cross_heads, 
                                                                  dim_head = cross_dim_head, 
                                                                  dropout = attn_dropout), 
                                             context_dim = latent_dim)
        get_rev_cross_ff = lambda: PreNorm(input_dim, FeedForward(input_dim, 
                                                                  dropout = ff_dropout))
        ...
        get_cross_attn, get_cross_ff, get_rev_cross_attn, get_rev_cross_ff, \
            get_input_attn, get_latent_attn, get_latent_ff = map(cache_fn, 
                                                                 (get_cross_attn, 
                                                                  get_cross_ff, 
                                                                  get_rev_cross_attn, 
                                                                  get_rev_cross_ff, 
    ...
    def forward(self, data, mask = None):
        ...
        for i, (cross_attn, cross_ff, rev_cross_attn, rev_cross_ff, input_attn, latent_attn, latent_ff) in \
